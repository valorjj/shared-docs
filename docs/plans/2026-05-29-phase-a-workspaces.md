...
> **✅ PHASE A COMPLETE (2026-06-01)** — tagged `phase-a-complete` on both repos (backend `f3dde75`, frontend `e770b20`). All 6 sections done.
>
> **Section 6 (validation) results:** backend `./gradlew build` green — 26 tests / 14 classes, all pass. Frontend `tsc --noEmit` + `npm run build` clean (pre-existing eslint debt remains in untouched calc/notes-editor files). Structural self-review: all 11 resource entities carry `workspace_id` + extend BaseEntity; 5 category tables intentionally global (Phase C); all 13 resource controllers + WorkspaceController use `@CurrentWorkspace`; no unscoped resource read paths (attachments guarded via parent note; categories global by design). Two-user HTTP isolation (Task 26) verified live on :8091: distinct workspaces, no cross-tenant leak, non-member→403, `visibility=WORKSPACE` round-trips. `main` has not advanced (v2 +22, main +0). Task 25 (browser happy-path) is owned by the user (manual). v2 backend also redeployed to Docker :8090 (healthy, Flyway at V12).
>
> **▶ NEXT: Phase B** — workspace create/rename UI, workspace settings, mobile switcher (the create-workspace modal + mobile chrome deferred from Section 5). Write `docs/plans/2026-06-…-phase-b-*.md` when starting.
>
> **Section 5 COMPLETE (frontend wiring + switcher + rename):** `workspaceStorage` (active id) → axios interceptor injects `X-Workspace-Id` + parses RFC 7807 problem+json; `api/workspaces` typed client + React Query hooks; `ActiveWorkspaceProvider` (mounted in main.tsx) resolves active workspace (stored→first), persists in render so the header is set before gated queries fire, and clears the query cache on switch; `MobileShell` gates the authed app on `ready`; `WorkspaceSwitcher` in TopNav (read + switch — create UI is Phase B); logout clears the active id. **SHARED→WORKSPACE rename DONE** across both stacks (backend Visibility enum + V12 migration + JPQL/DTO/test; frontend NoteVisibility type + list filters + editor toggle). Verified: backend tests green; frontend tsc + prod build clean (pre-existing eslint debt in untouched calc/notes-editor files remains). Frontend HEAD on `v2-multi-tenant`; backend HEAD includes V12.
> Not yet browser-E2E'd (needs local backend on 8091 + `VITE_API_BASE_URL` pointed at it + `npm run dev`). Mobile workspace switching deferred to Phase B (switcher is desktop TopNav only).
>
...
>
> **Section 4 is the long grind:** 10 resource features (note, sheet, calc, purchase+category, settlement, recurring, todo+category, anniversary+category, link+category, recipe), each: Flyway `V<n>__<feature>.sql` adding `workspace_id` + FK (`ON DELETE RESTRICT`) → entity extends BaseEntity + `workspaceId` field → repo filters by workspaceId → service threads it → controller takes `@CurrentWorkspace` → one isolation integration test. Commit per-feature. Task 10 is the template; 11–19 apply it.
> Decisions still open for Section 4 (ask user): (a) order to tackle features; (b) whether each feature also retrofits to BaseEntity now or keeps its existing id/createdAt (the v1 entities predate BaseEntity); (c) how aggressively to batch commits.
