...
- **Pane content:** on first open, call the lazy-create endpoint, then render the reused note editor body (full editing: chips, urls, formatting — the `NoteEditorBody` reuse pattern `SharedNoteView` already uses) with the `Comments` component below it, `pageId = "note-{noteId}"`. Body editing carries chips (including 계획/안건/선택지), URL paste/autolink, and all core formatting — the same SharedNoteView `minimal` profile the notes list already uses; upload-dependent features (image/file upload, file/link-card/snapshot slash entries, the link dialog) are not wired in this profile and remain available by opening the note directly from the notes list.
