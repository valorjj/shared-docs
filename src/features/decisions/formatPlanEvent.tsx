import { Flag, ListPlus, CirclePlus, CheckCircle2, RefreshCw, RotateCcw, Lock, LockOpen, CalendarClock, CalendarX, Trash2, ArrowUpRight, type LucideIcon } from 'lucide-react'
...
export function planEventText(e: PlanEvent, actor: string): string {
  ...
  switch (e.type) {
    ...
    case 'SUBDECISION_ADDED': return `하위결정 ${q(p.title)}을(를) 추가했어요`
    case 'SUBDECISION_REMOVED': return `하위결정 ${q(p.title)}을(를) 휴지통으로 보냈어요`
    case 'SUBPLAN_PROMOTED': return `안건 ${q(p.title)}을(를) 하위결정으로 전환했어요`
